import torch, torchvision, tqdm, ssl
...
def get_loaders(bs):
    ...
    train_set = torchvision.datasets.CIFAR100(root='./data', train=True, download=False, transform=tf_train)
    test_set = torchvision.datasets.CIFAR100(root='./data', train=False, download=False, transform=tf_test)
    # Windows 子进程上限 12，无 Graph 安全
...
# ---------- 4. 训练（AMP + 梯度累积） ----------
def train(student, teacher, loader, optimizer, criterion, device, scaler, accum=2):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        with autocast('cuda'):
            with torch.no_grad():
                t_out = teacher(x)
            s_out = student(x)
            loss = criterion(s_out, t_out, y) / accum
...
def main():
    ...
    batch_size = 896          # 5060Ti 16G 安全上限
